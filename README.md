...
Outputs: `go-version` (installed, without the `go` prefix), `go-root`, and `cache-hit`. Both `go` and `gos` are on `PATH` afterwards, so later steps can run `gos verify`, `gos status`, or `gos each 1.25,1.26 -- go test ./...`. The action works on `ubuntu`, `macos`, and `windows` runners (through Git Bash) and is exercised on all three by this repository's CI.
