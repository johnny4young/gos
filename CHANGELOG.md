...
- `gos.cmd` (the Windows shim) and `install.ps1` now look for Git for Windows' `bash.exe` in its install locations first and never accept the WSL launcher in `System32`, which `where bash.exe` returned first on machines with WSL enabled and which cannot run a Windows path. The shim's error now goes to stderr so it cannot corrupt `--json` output.
