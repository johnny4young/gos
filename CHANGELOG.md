...
- Test suites are discovered and run by `scripts/run-tests.bash` (every tracked `tests/*.bash`, per-OS rules in each suite's `# gos-suite:` header, parallel waves), so adding a suite is adding a file; `validate-local` and CI use it. The 3,100-line `tests/features.bash` is split into ten themed suites over a shared `tests/lib-features.bash`. New suites cover the wget download branch (never exercised before), a host without curl or wget, the side-by-side-link warning in flat mode, and property checks on version ordering.
