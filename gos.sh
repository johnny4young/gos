...
# here. Fields: name|scope|default|description; scope is gos, install.sh, or
# install.ps1. Descriptions must not contain "|".
_gos_env_manifest() {
  cat <<'GOS_ENV'
...
# Print the environment manifest for the surface generators; --json gives an
# array of {name, scope, default, description}.
cmd___env() {
  local arg first="true" name scope default description
  for arg in "$@"; do
    case "$arg" in
      --json) GOS_OUTPUT_JSON=1 ;;
      *)
  ...
  printf '{"variables":['
  _gos_env_manifest | while IFS='|' read -r name scope default description; do
    if [ "$first" = "true" ]; then
      first="false"
    else
      printf ','
