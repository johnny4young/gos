...
8. Run the local validation set. Start with the full orchestrator; it runs the
   required gates and any optional local tools that are installed. The expanded
   commands below keep release-candidate evidence explicit for CI/release parity.
   Use `scripts/validate-local.bash --required-only` only for non-release local
   triage, and note any optional local-tool skips in the PR.
